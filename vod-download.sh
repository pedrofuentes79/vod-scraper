...
dl_args=(
  ...
  # Write the metadata sidecar ("<stem>.info.json") next to the video. The ingest
...
#    together with each video's sidecar .m4a + .info.json (by exact path, never a glob).
mapfile -t vids < <(
...
for old in "${vids[@]:KEEP_LATEST}"; do
  ...
  rm -f -- "$old" "${old%.mp4}.m4a" "${old%.mp4}.info.json"
done
